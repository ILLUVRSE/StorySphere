...
class Renderer {
    ...
    private drawPlayers(state: GameState) {
        ...
        const drawPlayer = (x: number, y: number, color: string, isFielder = false) => {
             ctx.fillStyle = color;
             ctx.beginPath();
        ...
        const homeColor = RENDER_CONSTANTS.TEAM_HOME.primary;
        const awayColor = RENDER_CONSTANTS.TEAM_AWAY.primary;
        const defenseColor = state.isTopInning ? homeColor : awayColor;
        const offenseColor = state.isTopInning ? awayColor : homeColor;

        // Pitcher
        drawPlayer(FIELD.mound.x, FIELD.mound.y, defenseColor);

        // Batter
        drawPlayer(FIELD.homePlate.x - 30, FIELD.homePlate.y, offenseColor);

        // Fielders
        if (state.fielders) {
            state.fielders.forEach(f => {
                drawPlayer(f.position.x, f.position.y, defenseColor, true);
            });
        }
    }
